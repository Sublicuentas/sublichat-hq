    ...
    try {
      await context.route('**/*', async route => {
        try {
          ...
          if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || (url.port && !['80','443'].includes(url.port))) return route.abort();
          if (/^(?:localhost|metadata\.google\.internal)$|\.(?:localhost|local|internal)$/.test(url.hostname)) return route.abort();
          let cached = addresses.get(url.hostname);
          if (!cached || Date.now() - cached.at > 60000) {
            const values = net.isIP(url.hostname) ? [{ address: url.hostname }] : await dns.lookup(url.hostname, { all: true });
            cached = { at: Date.now(), allowed: values.length > 0 && values.every(x => publicAddress(x.address)) }; addresses.set(url.hostname, cached);
          }
          if (!cached.allowed) return route.abort();
          if (request.isNavigationRequest() && !request.frame().parentFrame() && !allowsNavigation(url.href, platform)) return route.abort();
