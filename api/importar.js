...
async function secBackupOne(db, seccion, body, motivo) {
  ...
  let totalBloques = 0;
  await ref.set({
    version: "backup-secciones-20260705",
    seccion,
...
async function secBackupCrear(db, body) {
  ...
  return { status: 200, json: { ok: true, created: items.length, items, motivo } };
}
...
async function handler(req, res) {
  ...
  if (req.method === "GET" || req.method === "HEAD") return res.status(200).json({ ok: true, version: "importar-cjs-sec-backups-20260705", msg: "api/importar activo", acciones: ["sec_estado","sec_leer","sec_hoja_iniciar","sec_hoja_bloque","sec_finalizar","sec_hoja_leer","sec_backup_crear","sec_backup_listar","sec_backup_restaurar","sec_backup_diario"] });
