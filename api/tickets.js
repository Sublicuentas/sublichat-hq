...
function normalizeDestinos(destino) {
  const d = clean(destino, 30).toLowerCase();
  if (d === 'both' || d === 'ambos') return ['sublicuentas', 'relojes'];
...
async function createTicket(db, body) {
  ...
  const destinos = normalizeDestinos(body.destino || body.destinos || 'sublicuentas');
  const item = {
    titulo,
    detalle,
    destinos,
    ...
    creadoPorRol: clean(body.rol || '', 40).toLowerCase(),
    createdAt: now,
    updatedAt: now,
    resolucion: '',
    resueltoPor: '',
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      return res.status(200).json({ ok: true, msg: 'api/tickets activo', version: 'tickets-auditoria-telegram-20260705' });
    }
