...
function normalizePromotion(raw = {}, previous = {}) {
  ...
  return {
    ...
    imagenModo: clean(raw.imagenModo != null ? raw.imagenModo : previous.imagenModo, 10) === 'contain' ? 'contain' : 'cover',
    imagenZoom: clampInteger(raw.imagenZoom != null ? raw.imagenZoom : previous.imagenZoom, 100, 250, 100),
...
async function publicPortal(db, req, res) {
  ...
  const promociones = promoSnap.docs
    ...
    .map(promo => ({
      ...
      imagenModo: promo.imagenModo === 'contain' ? 'contain' : 'cover',
